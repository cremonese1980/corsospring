...
public class Solution {
	...
	private static void getSolution() throws CloneNotSupportedException {
		...
		for (Node node : clone.getNodes()) {
			...
			if(node.isLeaf()){
				...
				findWay(node, node.getAvailableNodes(), solution);
			}
		}
		
		if(!leafPresent){
			
			for (Node node : tree.clone().getNodes()) {
	...
	private static void findWay(Node root, List<Node> currTree, List<Node> solution) throws CloneNotSupportedException {

		
		
		for (Node currentNode : currTree) {sfg
			...
			if(!solution.contains(currentNode)){
				...
				if(solution.size() == 0 || solution.get(solution.size()-1).getAvailableNodes().contains(currentNode)){
					...
					findWay(currentNode, currentNode.clone().getAvailableNodes(), new LinkedList<Node>(solution));
				}
...
class Tree implements Cloneable{
	...
	protected Tree clone() throws CloneNotSupportedException {
		...
		Tree clone = new Tree(getNodeSize(), getEdgeSize());
...
class Node implements Cloneable{
	...
	public boolean isLeaf(){
		return availableNodes.size() == 1;
	}
	public boolean isRelativeLeaf(List<Node> excludeList){
		
		int excludeSize = 0;
		
		for (Node node : excludeList) {
		...
		return availableNodes.size() - excludeSize  == 1;
	}
	...
	protected Node clone() throws CloneNotSupportedException {
		...
		Node clone = new Node(getIndex());
		
		for (Node node : availableNodes) {
			
			clone.addEdge(node.clone());
		}
